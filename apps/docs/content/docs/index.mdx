...
As forms almost always grow more complex, AutoForm gives you options to customize how forms are rendered (e.g. using the [`fieldConfig`](/docs/react/customization) option) and gives you escape hatches to customize the form even further. Use the [React Hook Form guide](/docs/react/getting-started) for the default React adapter, or the [TanStack Form guide](/docs/tanstack/getting-started) when you want TanStack Form state and composition APIs.

...
description: Build a custom UI integration for the TanStack Form AutoForm adapter.
---
...
export interface CustomAutoFormFieldComponents
  extends AutoFormFieldComponents {}
```
...
For adapter-neutral field components, use `useField` from `@acp-autoform/react`. AutoForm provides the correct implementation for the current adapter.
...
Use TanStack's field context only when your component needs TanStack-specific APIs:
...
export const BooleanField: React.FC<AutoFormFieldProps> = ({ inputProps, id }) => {

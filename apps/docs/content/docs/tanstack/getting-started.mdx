...
Install TanStack Form with the AutoForm React base package:

<ModeTab command="@acp-autoform/react @tanstack/react-form" />

If you use a published UI integration, the UI package depends on `@acp-autoform/react`, but `@tanstack/react-form` is still the form-engine peer dependency you should install in your app.
...
```tsx
import { AutoForm } from "@acp-autoform/ant/tanstack-form";
```
...
```tsx
import { AutoForm } from "@acp-autoform/chakra/tanstack-form";
```

If you want to integrate into your own UI components or another UI library, check out the [custom integration guide](/docs/tanstack/custom-integration).
...
For UI integrations and custom fields, prefer AutoForm's shared `useField` export from `@acp-autoform/react`. It connects to the active adapter, so the same field component can work with React Hook Form or TanStack Form.

When a custom component needs TanStack-specific field APIs, use `useFieldContext` from `@acp-autoform/react/tanstack-form`. See [customization](/docs/tanstack/customization#connecting-custom-fields-to-tanstack-form).

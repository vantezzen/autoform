...
`JoiProvider` exposes the original schema and schema type to `@dual-autoform/react`, so AutoForm can create a React Hook Form resolver automatically. You do not need to configure a Joi resolver manually when using the official provider.
...
Use the [`fieldConfig`](/docs/react/customization) function to customize how a field is rendered. Import it from `@dual-autoform/joi`.

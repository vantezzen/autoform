...
V1 refers to the original `shadcn/ui` auto-form component. V2 is the current `@autoform/*` package ecosystem.
...
npm install @autoform/zod react-hook-form @hookform/resolvers
```
...
Custom field components no longer receive `value` or `onChange` directly. Use `useField()` to get the react-hook-form field props, and treat `inputProps` as the props configured with `fieldConfig.inputProps`.

`useField()` returns the same object as `useController(...).field`. If you need the full `useController` return value, import `useController` from `react-hook-form` and pass the `id` from `AutoFormFieldProps` as the field name:

```tsx
const { field, fieldState } = useController({ name: id });
```

If you previously imported `buildZodFieldConfig`, `buildYupFieldConfig`, or `buildJoiFieldConfig` from `@autoform/react`, replace those helpers with `fieldConfig` from the matching schema package:

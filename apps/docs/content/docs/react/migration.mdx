...
V1 refers to the original `shadcn/ui` auto-form component. V2 is the current `@dual-autoform/*` package ecosystem.
...
npm install @dual-autoform/zod react-hook-form @hookform/resolvers
```
...
+ import { FieldTypes } from "@/components/ui/autoform/react-hook-form";
...
If you previously imported `buildZodFieldConfig`, `buildYupFieldConfig`, or `buildJoiFieldConfig` from `@dual-autoform/react`, replace those helpers with `fieldConfig` from the matching schema package:

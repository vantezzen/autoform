...
For complete source definitions, see [`packages/react/src/types.ts`](https://github.com/adityacodepublic/autoform/tree/tanstack-form-integration/packages/react/src/types.ts).

## AutoFormProps

<AutoFormPropsTable />

UI integrations such as `@dual-autoform/mui`, `@dual-autoform/ant`, etc. provide default uiComponents and formComponents, while still allowing you to override them. They provide AutoForm with `ExtendableAutoFormProps` type, which is the same as `AutoFormProps`, except uiComponents and formComponents props are optional.

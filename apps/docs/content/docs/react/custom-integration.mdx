...
`inputProps` contains props from `fieldConfig.inputProps`. Import the `useField` hook from `@acp-autoform/react` to connect the component to the active form adapter. `useField({ name: id })` returns a normalized `field` binding with `value`, `onChange`, `onBlur`, `name`, and an optional `ref`. This keeps the UI integration independent from React Hook Form and TanStack Form.
...
Create a new file called `src/AutoForm.tsx`. This example wraps the React Hook Form adapter and provides your custom UI components. Use the `tanstack-form` subpath when building the TanStack variant.

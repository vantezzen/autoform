...
```tsx
// src/types.ts
import { AutoFormUIComponents, AutoFormFieldComponents } from "@acp-autoform/react";
...
export const NumberField: React.FC<AutoFormFieldProps> = ({
  ...
  return <input id={id} type="number" {...inputProps} {...formField} />;
};
```
...
`inputProps` contains props from `fieldConfig.inputProps`. Use the `useField` prop to connect the component to react-hook-form. `useField()` returns the same object as `useController(...).field`: `value`, `onChange`, `onBlur`, `name`, `ref`, and related field props. If your component needs `fieldState`, `formState`, or any other part of the full `useController` API, import `useController` from `react-hook-form` and use the `id` from `AutoFormFieldProps` as the field name

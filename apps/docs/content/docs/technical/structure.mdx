...
## 2. Schema Providers (e.g., @dual-autoform/zod)

These packages adapt specific schema libraries to work with AutoForm. They implement the `SchemaProvider` interface defined in @dual-autoform/core.
...
- Expose the original schema and schema type so `@dual-autoform/react` can create React Hook Form resolvers

Example with @dual-autoform/zod:
...
If you build a custom schema provider for use with `@dual-autoform/react`, implement `schemaType` and `getSchema()` when you want AutoForm to create resolver-backed validation automatically:
...
## 4. UI-specific libraries (e.g., @dual-autoform/mui)

These packages implement UI components for specific design systems or component libraries. They use the core functionality from @dual-autoform/react and provide styled form elements.
...
Example with @dual-autoform/mui:
...
2. The schema is passed to the appropriate schema provider (e.g., @dual-autoform/zod).
3. The `AutoForm` component from a UI-specific library (e.g., @dual-autoform/mui) is used in the React application.
4. This `AutoForm` component wraps the core `AutoForm` from @dual-autoform/react, providing UI-specific components.

...
## 2. Schema Providers (e.g., @acp-autoform/zod)

These packages adapt specific schema libraries to work with AutoForm. They implement the `SchemaProvider` interface defined in @acp-autoform/core.
...
- Expose the original schema and schema type so `@acp-autoform/react` can create React Hook Form resolvers

Example with @acp-autoform/zod:
...
If you build a custom schema provider for use with `@acp-autoform/react`, implement `schemaType` and `getSchema()` when you want AutoForm to create resolver-backed validation automatically:
...
## 3. @acp-autoform/react

This package provides the shared React contracts, context, and adapter-agnostic hooks used by every React UI integration. Runtime form implementations are exposed through the `react-hook-form` and `tanstack-form` subpaths.
...
## 4. UI-specific libraries (e.g., @acp-autoform/mui)

These packages implement UI components for specific design systems or component libraries. They use the core functionality from @acp-autoform/react and provide styled form elements.
...
2. The schema is passed to the appropriate schema provider (e.g., @acp-autoform/zod).
3. The `AutoForm` component from a UI-specific library (e.g., @acp-autoform/mui) is used in the React application.
4. This `AutoForm` component wraps the core `AutoForm` from @acp-autoform/react, providing UI-specific components.
